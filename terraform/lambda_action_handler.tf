...
resource "aws_iam_policy" "lambda_handler_policy" {
  ...
  policy = templatefile("./policies/lambda_handler_policy.json.tpl", { account_id = var.account_id, launch_template_iam_role_name = var.launch_template_iam_role_name, aws_cloudwatch_log_group_handler_prefix = var.aws_cloudwatch_log_group_handler_prefix })
}
